...
type ContainerListPanelProps = {
  onConnectTerminal: (container: ContainerSummary) => void;
  onOpenContainerDetail: (container: ContainerSummary) => void;
  onCreateClick?: () => void;
};

export function ContainerListPanel({ onConnectTerminal, onOpenContainerDetail, onCreateClick }: ContainerListPanelProps) {
  ...
  const [deleteCandidate, setDeleteCandidate] = useState<ContainerSummary | null>(null);
  ...
  const requestDelete = (container: ContainerSummary) => {
    setOpenActionMenuName(null);
    setDeleteCandidate(container);
  };
    ...
    <>
                      ...
                      <td>
                        <div className={styles.tableActions}>
                          {isEditing ? (
                            ...
                            <>
                              <Button type="button" onClick={() => onOpenContainerDetail(container)} disabled={isBusy}>
                                상세
                              </Button>
                              <Button
                                type="button"
                                onClick={() => onConnectTerminal(container)}
                              ...
                              <div className={styles.actionMenu}>
                                ...
                                {isActionMenuOpen && (
                                  ...
                                  onClick={() => requestDelete(container)}
                                    >
                                      삭제
                                    </button>
                                  </div>
      ...
      <ConfirmDialog
        ...
        description={deleteCandidate
          ? `${deleteCandidate.displayName} 컨테이너와 실행 중인 프로세스를 제거합니다. 이 작업은 되돌릴 수 없습니다.`
          : ''}
        isOpen={deleteCandidate !== null}
        isSubmitting={busyContainerName === deleteCandidate?.containerName}
        onClose={() => setDeleteCandidate(null)}
        onConfirm={() => {
          if (!deleteCandidate) return;
          void handleDelete(deleteCandidate.containerName).then((deleted) => {

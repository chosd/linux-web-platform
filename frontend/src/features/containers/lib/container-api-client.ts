...
export async function createContainer(
  ...
  const response = await fetch(`${containerApiBaseUrl}/api/containers`, {
    ...
    body: JSON.stringify({
      ...
      networkAlias: networkAlias?.trim() || undefined
    })
  });
...
export async function listDockerNetworks() {
  const response = await fetch(`${containerApiBaseUrl}/api/networks`);
  if (!response.ok) {
    throw new Error(await parseErrorMessage(response, 'Docker network list request failed'));
  }
  return (await response.json()) as DockerNetwork[];
}

export async function createDockerNetwork(name: string) {
  const response = await fetch(`${containerApiBaseUrl}/api/networks`, {
    method: 'POST',
    headers: jsonHeaders,
  ...
  if (!response.ok) {
    throw new Error(await parseErrorMessage(response, 'Docker network create request failed'));
  }
  return (await response.json()) as DockerNetwork;
}

export async function changeContainerNetwork(
  containerName: string,
  networkName: string,
  action: 'connect' | 'disconnect'
) {
  const response = await fetch(
    `${containerApiBaseUrl}/api/containers/${encodeURIComponent(containerName)}/networks`,
    {
  ...
  if (!response.ok) {
    throw new Error(await parseErrorMessage(response, `Docker network ${action} request failed`));
  }
  return (await response.json()) as DockerNetwork;
}
